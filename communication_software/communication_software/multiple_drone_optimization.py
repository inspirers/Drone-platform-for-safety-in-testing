...
def getDronesLoc(coordslist, droneOrigin):
    ...
    max_x = [
        sample_coord.lat + (sample_coord.lat) / 16,
        sample_coord.lng + (sample_coord.lng) / 16,
    ]
    max_y = [
        sample_coord.lat + (sample_coord.lat) / 17,
        sample_coord.lng + (sample_coord.lng) / 17,
    ]
    min_x = [
        sample_coord.lat + (sample_coord.lat) / 15,
        sample_coord.lng + (sample_coord.lng) / 15,
    ]
    min_y = [
        sample_coord.lat + (sample_coord.lat) / 14,
        sample_coord.lng + (sample_coord.lng) / 14,
    ]
    ...
    for coordList in coordslist.values():
        for coord in coordList:
            ...
            if x > max_x[0] and [x, y] not in (max_y, min_x, min_y):
                max_x = [x, y]
            if y > max_y[1] and [x, y] not in (max_x, min_x, min_y):
                max_y = [x, y]
            if x < min_x[0] and [x, y] not in (max_x, max_y, min_y):
                min_x = [x, y]
            if y < min_y[1] and [x, y] not in (max_x, max_y, min_x):
                min_y = [x, y]
    ...
    def vert1(x):
        return (
            (p4[1] - p1[1]) / (p4[0] - p1[0]) * x
            + p1[1]
            - p1[0] * (p4[1] - p1[1]) / (p4[0] - p1[0])
        )

    def vert2(x):
        return (
            (p2[1] - p1[1]) / (p2[0] - p1[0]) * x
            + p1[1]
            - p1[0] * (p2[1] - p1[1]) / (p2[0] - p1[0])
        )
    ...
    def vert3(x):
        ...
        # print("p3[1]: ", p3[1])
        return (
            (p3[1] - p2[1]) / (p3[0] - p2[0]) * x
            + p2[1]
            - p2[0] * (p3[1] - p2[1]) / (p3[0] - p2[0])
        )

    def vert4(x):
        return (
            (p3[1] - p4[1]) / (p3[0] - p4[0]) * x
            + p4[1]
            - p4[0] * (p3[1] - p4[1]) / (p3[0] - p4[0])
        )
    ...
    def drone_line_2_4(x):
        return (
            (dp4[1] - dp2[1]) / (dp4[0] - dp2[0]) * x
            + dp2[1]
            - dp2[0] * (dp4[1] - dp2[1]) / (dp4[0] - dp2[0])
        )
    ...
    def drone_line_1_3(x):
        return (
            (dp3[1] - dp1[1]) / (dp3[0] - dp1[0]) * x
            + dp1[1]
            - dp1[0] * (dp3[1] - dp1[1]) / (dp3[0] - dp1[0])
        )
    ...
    drone_loc_x1, drone_loc_y1, drone_loc_x2, drone_loc_y2 = find_drone_point_2_4(
