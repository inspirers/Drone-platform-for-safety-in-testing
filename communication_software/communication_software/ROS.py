...
class AtosCommunication(Node):
    ...
    def __init__(self):
        ...
        self.reset_test_objects_pub = self.create_publisher(Empty, '/atos/reset_test_objects', self.QOS)
        self.reload_object_settings_pub = self.create_publisher(Empty, '/atos/reload_object_settings', self.QOS)
        ...
        self.get_traj = self.create_client(GetObjectTrajectory, '/atos/get_object_trajectory')

        # This is the ROS service for getting the test origin from ATOS
        self.get_test_origin_client = self.create_client(GetTestOrigin, '/atos/get_test_origin')
    ...
    def start_coordinate_subscriber(self):
        ...
        for id in self.object_coordinates.keys():
            ...
            coordinate_subscribers.append(
                self.create_subscription(NavSatFix, topic, lambda msg: self.coordinate_callback(msg, id), self.QOS))
    ...
    def get_test_origin_callback(self):
        ...
        if not response.success:
            ...
            self.test_origin = Coordinate(response.origin.position.latitude,
    ...
    def get_origin_coordinates(self):
        ...
        if self.state == "UNDEFINED":
            ...
            self.get_logger().error('ATOS in state: ' + self.state + ', unable to get coordinates.')
            return None
        elif self.state == "IDLE":
            self.get_logger().error('ATOS is IDLE, the state has to be "initialized" to get coordinates.')
            return None
